...
function NewsContent() {
    ...
    const handleSearch = (q?: string) => {
        ...
        if (term.trim()) {
            router.push(`/news?s=${encodeURIComponent(term.trim())}&scraper=${scraper}`);
            ...
            <div className="max-w-6xl mx-auto px-3 sm:px-4 py-5 sm:py-6">
                {loading ? (
                    <div className="space-y-3">
                        {[...Array(8)].map((_, i) => (
                            <div key={i} className="space-y-2 rounded-lg p-4 bg-[#0f1525]/70 border border-white/5">
                    ...
                    <div className="space-y-3">
                        {results.map((news, i) => (
                            <article key={i} className="group rounded-lg p-4 bg-[#0f1525]/70 border border-white/5 hover:border-white/10 transition fade-slide" style={{ animationDelay: `${i * 50}ms` }}>
                                <a href={news.url} target="_blank" rel="noopener noreferrer" className="block space-y-2">
                                    <div className="flex items-center gap-2 text-xs text-[#8da0bf]">
                                        {getFavicon(news.url) && (
                                            <img
                                                src={getFavicon(news.url)!}
                                                alt=""
                                                className="w-4 h-4"
                                                loading="lazy"
                                        ...
                                        <span className="truncate">{news.source?.name || news.url}</span>
                                    </div>
                                    <h2 className="text-[#c9d7ff] group-hover:underline text-lg">
                                        {news.title}
                                    </h2>
                                    {news.description && (
                                        <p className="text-[#c7d2e4] text-sm leading-relaxed line-clamp-2">{news.description}</p>
                                    )}
                                    <div className="flex gap-2 text-xs text-[#8da0bf]">
