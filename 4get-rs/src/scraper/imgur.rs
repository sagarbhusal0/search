...
#[async_trait]
impl Scraper for Imgur {
    fn name(&self) -> &str {
        "imgur"
    }

    async fn image(&self, query: &SearchQuery) -> Result<ImageResponse, AppError> {
        let url = format!("https://imgur.com/search?q={}", urlencoding(&query.q));
        ...
        let img_sel = Selector::parse("img[src*='imgur'], img.image").unwrap();
        let parent_sel = Selector::parse("div.post, a[href*='//imgur.com']").unwrap();
